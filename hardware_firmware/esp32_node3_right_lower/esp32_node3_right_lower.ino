...
void setup() {
  ...
  // Initialize Node ID (Change per node: 1, 2, or 3)
  myData.id = 3;

  // Initialize I2C First (Before Wi-Fi radio causes power spikes)
  // Use 50kHz SMBus clock frequency for optimal MLX90614 stability
  ...
  if (devCount == 0) {
    Serial.println(" -> CRITICAL: No I2C devices detected! Check 3.3V and GND rails.");
  }
  Serial.println("-------------------------------\n");
  ...
  if (readMLX90614(initTemp)) {
    ...
    Serial.print("SUCCESS: MLX90614 detected! Initial Temp: ");
...
void loop() {
  ...
  Serial.print("Node 3 (R_Low) | Shank_Accel(X,Y,Z): "); 
